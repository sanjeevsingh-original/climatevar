...
def _validate_daily(data, dim, *, min_daily_valid_fraction=0.9):
    """Convert to daily amounts and reject/flag incomplete sub-daily days."""
    if not 0 <= min_daily_valid_fraction <= 1:
        raise ValueError("min_daily_valid_fraction must be between 0 and 1.")
    try:
        frequency = infer_frequency(data.rename({dim: "time"}) if dim != "time" else data)
    except ValueError:
        frequency = "irregular"
    daily = daily_amount(data, dim)
    if min_daily_valid_fraction == 0 or frequency == "daily":
        return daily
    expected = {"hourly": 24, "3-hourly": 8, "6-hourly": 4}.get(frequency)
    if expected is None:
        if frequency == "irregular":
            raise ValueError("ETCCDI indices require a regular daily/sub-daily time axis; aggregate irregular data to daily totals first.")
        return daily
    counts = data.resample({dim: "1D"}).count(dim=dim)
    coverage = counts / expected
    return daily.where(coverage >= min_daily_valid_fraction)
...
def _complete_year_mask(data, dim, min_valid_fraction):
...
def r10mm(data, dim="time", threshold=10.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    data = _validate_daily(data, dim, min_daily_valid_fraction=min_daily_valid_fraction)
    threshold = _validate_threshold(threshold, "threshold"); complete = _complete_year_mask(data, dim, min_valid_fraction)
...
def sdii(data, dim="time", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Simple daily precipitation intensity index, mm per wet day."""
    data = _validate_daily(data, dim, min_daily_valid_fraction=min_daily_valid_fraction)
    threshold = _validate_threshold(wet_day_threshold, "wet_day_threshold"); complete = _complete_year_mask(data, dim, min_valid_fraction)
    wet = data.where(data >= threshold)
    total = wet.groupby(f"{dim}.year").sum(dim=dim, skipna=True)
    count = wet.groupby(f"{dim}.year").count(dim=dim)
    result = total / count
    result.attrs.update({"units": "mm/day", "climatevar:wet_day_threshold_mm": threshold})
...
def wet_day_count(data, dim="time", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Annual number of wet days using the ETCCDI RR >= 1 mm convention."""
    data = _validate_daily(data, dim, min_daily_valid_fraction=min_daily_valid_fraction)
    threshold = _validate_threshold(wet_day_threshold, "wet_day_threshold"); complete = _complete_year_mask(data, dim, min_valid_fraction)
    return (data >= threshold).groupby(f"{dim}.year").sum(dim=dim, skipna=True).where(complete)
...
def r95p(data, reference=None, dim="time", baseline_start="1961-01-01", baseline_end="1990-12-31", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Annual precipitation above the ETCCDI 95th wet-day percentile."""
...
def r99p(data, reference=None, dim="time", baseline_start="1961-01-01", baseline_end="1990-12-31", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Annual precipitation above the ETCCDI 99th wet-day percentile."""
...
def r95p_fraction(data, reference=None, dim="time", baseline_start="1961-01-01", baseline_end="1990-12-31", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Percentage of annual precipitation contributed by R95p days."""
    numerator = r95p(data, reference, dim, baseline_start, baseline_end, wet_day_threshold, min_valid_fraction, min_daily_valid_fraction)
    denominator = prcptot(data, dim, wet_day_threshold, min_valid_fraction, min_daily_valid_fraction=min_daily_valid_fraction)
    result = 100 * numerator / denominator
    result.attrs.update({"units": "%", "climatevar:definition": "100 * R95p / PRCPTOT"})
    return result
...
def r99p_fraction(data, reference=None, dim="time", baseline_start="1961-01-01", baseline_end="1990-12-31", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    """Percentage of annual precipitation contributed by R99p days."""
    numerator = r99p(data, reference, dim, baseline_start, baseline_end, wet_day_threshold, min_valid_fraction, min_daily_valid_fraction)
    denominator = prcptot(data, dim, wet_day_threshold, min_valid_fraction, min_daily_valid_fraction=min_daily_valid_fraction)
    result = 100 * numerator / denominator
    result.attrs.update({"units": "%", "climatevar:definition": "100 * R99p / PRCPTOT"})
    return result
...
def _max_run(condition, dim, complete):
    result = condition.groupby(f"{dim}.year").map(lambda block: xr.apply_ufunc(_max_consecutive_1d, block, input_core_dims=[[dim]], output_core_dims=[[]], vectorize=True, dask="parallelized", output_dtypes=[int]))
    return result.where(complete)


def cwd(data, dim="time", wet_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    data = _validate_daily(data, dim, min_daily_valid_fraction=min_daily_valid_fraction); threshold = _validate_threshold(wet_day_threshold, "wet_day_threshold")
...
def cdd(data, dim="time", dry_day_threshold=1.0, min_valid_fraction=0.9, *, min_daily_valid_fraction=0.9):
    data = _validate_daily(data, dim, min_daily_valid_fraction=min_daily_valid_fraction); threshold = _validate_threshold(dry_day_threshold, "dry_day_threshold")
    return _max_run(data < threshold, dim, _complete_year_mask(data, dim, min_valid_fraction))
