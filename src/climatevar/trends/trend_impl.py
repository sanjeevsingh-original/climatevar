...
def _mk_stats(y):
    ...
    if np.isfinite(tau) and np.isclose(tau,1.0):tau=1.0
    elif np.isfinite(tau) and np.isclose(tau,-1.0):tau=-1.0
    s=float(sum(np.sign(y[j]-y[i]) for i in range(n) for j in range(i+1,n)))
    return s,float(tau),float(p),n

def _sen_1d(v):
    y=_clean(v); return float(theilslopes(y,np.arange(y.size)).slope) if y.size>=2 else np.nan
...
def _mk_variance(y):
    n=y.size; _,counts=np.unique(y,return_counts=True); tie=np.sum(counts*(counts-1)*(2*counts+5)); return (n*(n-1)*(2*n+5)-tie)/18.0
...
def _modified_mk_1d(v,max_lag=None):
    ...
    factor=max(1.0+(2.0/n)*np.sum((n-np.arange(1,len(r)+1))*r),1e-6); n_eff=float(np.clip(n/factor,1.0,n)); var_s=_mk_variance(y)*n/n_eff
...
def _block_indices(n,block_length,rng):
    ...
    starts=rng.integers(0,n-block_length+1,size=int(np.ceil(n/block_length))); return np.concatenate([np.arange(s,s+block_length) for s in starts])[:n]
...
def _sen_slope_ci_1d(v,alpha=0.05,n_resamples=1000,block_length=None,random_state=0):
    ...
    if not 0<alpha<1 or n_resamples<1:raise ValueError("alpha must be in (0,1) and n_resamples must be at least 1")
    if block_length is None:block_length=max(1,int(round(n**(1/3))))
    est=_sen_1d(y); t=np.arange(n,dtype=float); fitted=np.median(y-t*est)+est*t; residual=y-fitted
    rng=np.random.default_rng(random_state); boot=np.empty(n_resamples)
    # Resample detrended residuals, then reconstruct the fitted trend. This
    # preserves the estimated trend while retaining short-range dependence.
    for i in range(n_resamples):
        rb=residual[_block_indices(n,block_length,rng)]; boot[i]=_sen_1d(fitted+rb)
...
def mann_kendall(data,dim="time"):
    ...
    r=xr.apply_ufunc(_mk_stats,data,input_core_dims=[[dim]],output_core_dims=[[],[],[],[]],vectorize=True,dask="parallelized",output_dtypes=[float,float,float,int]); return xr.Dataset({"s":r[0],"tau":r[1],"pvalue":r[2],"n":r[3]})

def modified_mann_kendall(data,dim="time",max_lag=None):
    if dim not in data.dims:raise ValueError(f"Dimension {dim!r} is not present in the input data.")
    r=xr.apply_ufunc(_modified_mk_1d,data,kwargs={"max_lag":max_lag},input_core_dims=[[dim]],output_core_dims=[[],[],[],[],[]],vectorize=True,dask="parallelized",output_dtypes=[float,float,float,float,float]); return xr.Dataset({"s":r[0],"tau":r[1],"pvalue":r[2],"n_eff":r[3],"n":r[4]})
...
def trend_free_prewhitening(data,dim="time"):
    ...
    def tfpw(v):
        ...
        slope=_sen_1d(y); t=np.arange(y.size,dtype=float); d=y-slope*t; z=d-d.mean(); denom=np.sum(z[:-1]**2); r1=np.sum(z[1:]*z[:-1])/denom if denom>0 else 0.0; residual=d[1:]-r1*d[:-1]; return np.r_[residual[0],residual+slope*np.arange(1,y.size)]
...
def sens_slope_ci(data,dim="time",alpha=0.05,n_resamples=1000,block_length=None,random_state=0):
    ...
    r=xr.apply_ufunc(_sen_slope_ci_1d,data,kwargs={"alpha":alpha,"n_resamples":n_resamples,"block_length":block_length,"random_state":random_state},input_core_dims=[[dim]],output_core_dims=[[],[],[],[]],vectorize=True,dask="parallelized",output_dtypes=[float,float,float,float]); return xr.Dataset({"sen_slope":r[0],"ci_lower":r[1],"ci_upper":r[2],"n":r[3]}).assign_attrs(method="moving-block bootstrap percentile interval on detrended residuals",alpha=alpha,confidence_level=1-alpha,n_resamples=n_resamples,block_length="automatic" if block_length is None else block_length,random_state=random_state,slope_units="per observation step")
