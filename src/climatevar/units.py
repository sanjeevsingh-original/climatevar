...
def convert_units(data: xr.DataArray, target: str, *, source_units: str | None = None) -> xr.DataArray:
    ...
    if not src: raise ValueError(f"No source units supplied for {data.name!r}.")
    target = target.strip(); out = data.copy()
    if target == "K":
        ...
        # 1 mm of water is 1 kg m-2, so only the time denominator changes.
        if src in {"kg m-2 s-1", "kg/m2/s", "kg m-2 s^-1", "mm s-1", "mm/s"}: values = out
        elif src in {"mm day-1", "mm/day", "mm d-1"}: values = out / 86400.0
        elif src in {"mm h-1", "mm/hour", "mm hr-1"}: values = out / 3600.0
        elif src in {"mm min-1", "mm/min"}: values = out / 60.0
        ...
        values = out
    else: raise ValueError(f"Unsupported conversion from {source_units!r} to {target!r}.")
...
def normalize_units(ds: xr.Dataset, *, variables: list[str] | None = None) -> xr.Dataset:
    out = ds.copy(); selected = variables or list(SI_UNITS)
    for name in selected:
        ...
        if name == "precipitation" and _is_precip_flux(_unit_text(out[name].attrs["units"])):
            out[name] = convert_units(out[name], "kg m-2 s-1"); out[name].attrs["climatevar:quantity"] = "precipitation_flux"
        else:
            out[name] = to_si(out[name], name)
            if name == "precipitation": out[name].attrs["climatevar:quantity"] = "precipitation_amount"
    history = list(out.attrs.get("climatevar:normalization", [])); history = [history] if isinstance(history, str) else history
    history.append("normalize_units(target=SI_except_precipitation_amount_mm)"); out.attrs["climatevar:normalization"] = history
    return out
