...
COORD_ALIASES = {
    "latitude": ("latitude", "lat", "Latitude", "LATITUDE", "nav_lat", "y_lat", "XLAT", "XLAT_M", "XLAT_U", "XLAT_V"),
    "longitude": ("longitude", "lon", "Longitude", "LONGITUDE", "nav_lon", "x_lon", "XLONG", "XLONG_M", "XLONG_U", "XLONG_V"),
    "time": ("time", "Time", "TIME", "valid_time", "datetime", "date", "Date", "Times"),
}
VARIABLE_ALIASES = {
    "precipitation": ("precipitation", "precip", "precipitationCal", "precipitationCalES", "pr", "tp", "rain", "rainfall", "rf", "APCP_sfc", "RAINC", "RAINNC", "RAINSH"),
    "temperature": ("temperature", "temp", "t2m", "TMP_2m", "TMP_sfc", "T2", "tas", "air_temperature"),
    "surface_pressure": ("surface_pressure", "sp", "ps", "psl", "PRES_sfc", "PSFC", "pres", "pressure"),
    "relative_humidity": ("relative_humidity", "rh", "RH2", "RH_2m", "RH_prl", "hur", "r"),
    "specific_humidity": ("specific_humidity", "q", "q2", "Q2", "hus"),
    "u_wind": ("u_wind", "u10", "U10", "UGRD_10m", "UGRD_prl", "ua", "uwnd", "u"),
    "v_wind": ("v_wind", "v10", "V10", "VGRD_10m", "VGRD_prl", "va", "vwnd", "v"),
    "geopotential": ("geopotential", "z", "gh", "zg", "HGT_prl", "HGT", "GHT"),
}
DATASET_PRESETS = {
    "era5": {"latitude": "latitude", "longitude": "longitude", "time": "time", "precipitation": "tp", "temperature": "t2m", "surface_pressure": "sp"},
    "era5-land": {"latitude": "latitude", "longitude": "longitude", "time": "time", "precipitation": "tp"},
    "imerg": {"latitude": "lat", "longitude": "lon", "time": "time", "precipitation": "precipitation"},
    "gpm": {"latitude": "lat", "longitude": "lon", "time": "time", "precipitation": "precipitation"},
...
def normalize_coords(ds: xr.Dataset, *, dataset: str | None = None, strict: bool = False) -> xr.Dataset:
    """Rename common latitude/longitude/time coordinates to climatevar names."""
    ds = _wrf_times(ds)
...
def standardize_variables(ds: xr.Dataset, *, variables: Mapping[str, str] | None = None, dataset: str | None = None, strict: bool = False) -> xr.Dataset:
    """Rename common physical variables to canonical climatevar names."""
    mapping = dict(variables or {})
    preset = DATASET_PRESETS.get((dataset or "").lower(), {})
    names = list(ds.data_vars); renames = {}
    for canonical, aliases in VARIABLE_ALIASES.items():
        ...
        if source and source != canonical: renames[source] = canonical
