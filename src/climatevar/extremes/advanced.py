...
def gpd_model_comparison(excesses, x, dim="time"):
    ...
    return xr.Dataset({
        "stationary_shape": xi, "stationary_scale": scale,
        "nonstationary_shape": float(fit.x[0]), "log_scale_intercept": float(fit.x[1]),
        "log_scale_covariate": float(fit.x[2]), "stationary_loglik": ll_s,
        "nonstationary_loglik": ll_ns, "stationary_aic": aic_s, "nonstationary_aic": aic_ns,
        "stationary_bic": bic_s, "nonstationary_bic": bic_ns, "lr_statistic": lr,
        "lr_pvalue_asymptotic": float(chi2.sf(lr, 1)), "n_exceedances": n,
    })
...
def gpd_model_comparison_bootstrap(excesses, x, n_resamples=500, alpha=0.05, random_state=0):
    ...
    y, _, zs = _clean_pair(excesses, x)
    base = gpd_model_comparison(xr.DataArray(y), xr.DataArray(zs))
...
def gpd_nonstationary_ci(excesses, x, alpha=0.05, n_resamples=500, random_state=0):
    ...
    return xr.Dataset({
        "shape": float(fit.x[0]), "log_scale_intercept": float(fit.x[1]), "log_scale_covariate": float(fit.x[2]),
        "shape_ci_lower": float(lo[0]), "shape_ci_upper": float(hi[0]),
        "log_scale_intercept_ci_lower": float(lo[1]), "log_scale_intercept_ci_upper": float(hi[1]),
        "log_scale_covariate_ci_lower": float(lo[2]), "log_scale_covariate_ci_upper": float(hi[2]),
        "n_exceedances": int(y.size), "n_resamples": n_resamples, "n_valid_bootstrap": int(pars.shape[0]), "alpha": alpha,
    })
...
def pot_threshold_bootstrap(data, thresholds, return_period, n_resamples=500, alpha=0.05, random_state=0):
    ...
    Each replicate refits every candidate threshold and selects the candidate
    with the smallest AIC. The resulting percentile interval is conditional on
    this explicit selection rule and is therefore a reproducible probability
    interval, not a probability distribution over thresholds.
    """
    ...
    y = np.asarray(data.values, float).ravel()
    y = y[np.isfinite(y)]
    if y.size < 20:
        raise ValueError("at least 20 finite observations are required.")
    rng = np.random.default_rng(random_state)
    selected = np.full(n_resamples, np.nan)
    selected_threshold = np.full(n_resamples, np.nan)
    for b in range(n_resamples):
        sample = rng.choice(y, size=y.size, replace=True)
        counts, rates, shapes, scales, _ = _threshold_table(sample, t)
        aics = np.full(t.size, np.inf)
        for j, u in enumerate(t):
            e = sample[sample > u] - u
            if e.size >= 5 and np.isfinite(shapes[j]) and np.isfinite(scales[j]):
                ll = np.sum(genpareto.logpdf(e, shapes[j], loc=0.0, scale=scales[j]))
                aics[j] = 4 - 2 * ll
        j = int(np.argmin(aics))
        if np.isfinite(aics[j]):
            selected_threshold[b] = t[j]
            selected[b] = _return_level(shapes[j], scales[j], t[j], rates[j], return_period)
    ...
    q = np.quantile(selected[ok], [alpha / 2, 0.5, 1 - alpha / 2])
    valid_t = selected_threshold[np.isfinite(selected_threshold)]
    return xr.Dataset({
        "return_level_median": float(q[1]), "ci_lower": float(q[0]), "ci_upper": float(q[2]),
        "selected_threshold_median": float(np.median(valid_t)), "n_valid_bootstrap": int(ok.sum()),
