...
def _regrid(reference, candidate, method):
    ...
    if method in {"linear", "nearest", "nearest_s2d"}:
        if candidate.lat.ndim != 1 or candidate.lon.ndim != 1:
            raise ValueError("xarray linear/nearest interpolation requires 1-D lat/lon. Use xESMF bilinear or conservative for WRF curvilinear grids.")
...
def run_experiment(config: ExperimentConfig) -> ExperimentResult:
    ...
    for spec in config.products:
        products[spec.name] = _regrid(reference, _period(spec.load(), config.start, config.end), config.regrid_method or config.common_grid_method)
