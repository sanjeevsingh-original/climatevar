...
from .trend import modified_mann_kendall, sens_slope, sens_slope_ci, _mk_stats
...
def regional_slope_ci(
    ...
    """Estimate a regional Sen slope and bootstrap confidence interval.

    The spatial field is first reduced to one area-weighted regional series;
    bootstrap resampling is then performed on that series. This preserves the
    regional weighting and avoids treating grid cells as independent
    observations.
    """
    if not 0 < alpha < 1:
        raise ValueError("alpha must be between 0 and 1")
    series = regional_mean(data, lat_dim=lat_dim, lon_dim=lon_dim, weights=weights)
    result = sens_slope_ci(
        series,
        dim=time_dim,
        alpha=alpha,
    ...
    result.attrs.update({
        "weighting": "user-supplied cell areas" if weights is not None else "cosine latitude",
        "regional_series": "area-weighted mean before bootstrap",
    })
    return result
...
def field_significance(
    ...
    moving-block resampling of detrended residual fields. A common set of
    time indices is applied to every grid cell, preserving the spatial covariance
...
__all__ = ["regional_mean", "regional_trend", "regional_slope_ci", "field_significance"]
