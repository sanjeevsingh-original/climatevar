...
def calendar_name(data: xr.DataArray | xr.Dataset) -> str:
    ...
    index = time.to_index()
    if hasattr(index, "calendar"): return str(index.calendar)
    return "standard"
...
def time_step_seconds(data: xr.DataArray | xr.Dataset) -> xr.DataArray:
    """Return successive time-step durations in seconds; first value is NaN."""
    t = data["time"]
    if t.size < 2: return xr.full_like(t, np.nan, dtype=float)
    values = t.values
...
def infer_frequency(data: xr.DataArray | xr.Dataset, *, tolerance: float = 0.02) -> str:
    ...
    if not len(dt): raise ValueError("At least two valid time coordinates are required.")
